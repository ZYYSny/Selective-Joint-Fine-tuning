...
#include "caffe/layers/random_crop_boosted_data_layer.hpp"
...
std::vector<string> refined_randomcrop_boosted_joint_training_data_split(const string &s, const string &seperator) {
...
RandomCropBoostedDataLayer<Dtype>::~RandomCropBoostedDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void RandomCropBoostedDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  while(!input_target_nn_config_file.eof()){
    ...
    if(line=="")continue;
    vector<string> temp_split_str = refined_randomcrop_boosted_joint_training_data_split(line,"\t ");
...
template <typename Dtype>
void RandomCropBoostedDataLayer<Dtype>::ShuffleVectors(vector<int> &image_list) {
...
template <typename Dtype>
void RandomCropBoostedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < lines_size; ++item_id) {
    ...
    cv::Mat raw_img = cv::imread(root_folder + batch_lines_[item_id].first, is_color);
    CHECK(raw_img.data) << "Could not load " << batch_lines_[item_id].first;
...
template <typename Dtype>
void RandomCropBoostedDataLayer<Dtype>::InitRand() {
  const unsigned int rng_seed = caffe_rng_rand();
  rng_.reset(new Caffe::RNG(rng_seed));
}

template <typename Dtype>
int RandomCropBoostedDataLayer<Dtype>::Rand(int n) {
  CHECK(rng_);
  CHECK_GT(n, 0);
  caffe::rng_t* rng =
...
INSTANTIATE_CLASS(RandomCropBoostedDataLayer);
REGISTER_LAYER_CLASS(RandomCropBoostedData);
